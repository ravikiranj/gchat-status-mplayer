...
class MusicPlayerStatus:
    def __init__(self):
        ...
        self.supportedMediaPlayers = ["guayadeque", "banshee"] # Supported Media Players
    ...
    def Connect(self, name):
        ...
        if("guayadeque" in name):
            player_o = self.bus.get_object(name, "/Player")
        elif("banshee" in name):
            player_o = self.bus.get_object(name, "/org/bansheeproject/Banshee/PlayerEngine")
        else:
            player_o = self.bus.get_object(name, "")


        if("guayadeque" in name):
            self.player = dbus.Interface(player_o, "org.freedesktop.MediaPlayer")
        elif("banshee" in name):
            self.player = dbus.Interface(player_o, "org.bansheeproject.Banshee.PlayerEngine")
        ...
        if("guayadeque" in name):
            ...
            player_o.connect_to_signal("StateChanged", self.TrackChange, dbus_interface="org.bansheeproject.Banshee.PlayerEngine")
    ...
    def TrackChange(self, Track):
        ...
        title = ""
        year = ""
        if("banshee" in self.identity):
            status = self.player.GetCurrentState()
            if(status == "playing"):
                Track = self.player.GetCurrentTrack()
                title = self.getTrackProperty(Track, 'name')
                ...
                return
        elif("guayadeque" in self.identity):
            artist = self.getTrackProperty(Track, 'artist')
